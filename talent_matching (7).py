...
# ----------------------------
# Default directory for jobs base when not provided
# ----------------------------
base_dir_default = Path("data")

# Global helper to render segmented control or radio fallback.
def segmented_or_radio(label: str, options: list[str], index: int = 0):
    """
    Prefer Streamlit's segmented_control when available. Fall back to radio.
    Handles differences in the API across Streamlit versions.
    """
    if hasattr(st, "segmented_control"):
        # Try multiple argument names to support different versions.
        # The newest API uses 'default' as the selected option.
        try:
            return st.segmented_control(label=label, options=options, default=options[index])
        except TypeError:
            pass
        # Older API may use 'selection'
        try:
            return st.segmented_control(label=label, options=options, selection=options[index])
        except TypeError:
            pass
        # Fallback to 'value' parameter
        try:
            return st.segmented_control(label=label, options=options, value=options[index])
        except Exception:
            pass
    # Fallback: horizontal radio buttons
...
def _render_form(req_text_clean: str, job_row, job_id: str) -> None:
    ...
    # Catálogos de níveis de idiomas e formação
    levels_display = [
        "Sem conhecimento",
        "Básico",
        "Intermediário",
    ...
    # Função auxiliar para rotular níveis
    ...
    # Se não submetido, interrompe fluxo
    if not submitted:
        st.stop()
    ...
    # Monta lista consolidada de skills
    ...
    # Requisitos da vaga a partir do job_row
    ...
    # Estilos e componentes visuais dos cartões de comparação
    st.markdown(
        """
        <style>
            .match-grid {
    ...
    # Funções locais para construção de cartões
    cards_html: list[str] = []
    ...
    # Define os cartões de comparação
    ...
    # Exibe cartões e lista de skills
    ...
    # Carrega base para recomendação de vagas
    try:
    ...
    # Calcula pontuações para outras vagas
    suggestions = []
